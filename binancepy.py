...
class MarketData:
    ...
    def __init__(self,
                 exchange: str = 'perpetual',
                 testnet: bool = False,
                 symbol: str = 'btcusdt'):

        '''

        To use Delivery Binance Futures API  -> exchange = 'delivery'
        ...
        if self.exchange=='perpetual':
            if testnet == True:
                self.http_way = 'http://testnet.binancefuture.com/fapi/v1/'
            else:
                self.http_way = 'http://fapi.binance.com/fapi/v1/'
            self.symbol = symbol.lower()
        elif self.exchange=='delivery':
            if testnet == True:
                self.http_way = 'http://testnet.binancefuture.com/dapi/v1/'
            else:
                self.http_way = 'http://dapi.binance.com/dapi/v1/'
    ...
    def candles_data(self,
        ...
        return requests.get(f'{self.http_way}klines?{query}').json()
    ...
    def open_interest(self):
        return requests.get(f'{self.http_way}openInterest').json()

    def open_interest_statistics(self,
                                 period : str = '5m',
                                 startTime : int = None,
                                 endTime : int = None,
                                 limit : int = 500):
        if self.exchange=='perpetual':
            base_url = self.http_way.replace('fapi/v1/', 'futures/data/')
            params = {  'symbol' : self.symbol,
                        'period' : period,
                        'startTime' : startTime,
                        'endTime' : endTime,
                        'limit' : limit }
            query = urllib.parse.urlencode(dict([(k, v) for (k, v) in iter(params.items()) if v]))
            return requests.get(f'{base_url}openInterestHist?{query}').json()
        else:
            return {}
...
class Client:
    def __init__(self,
        ...
        elif exchange=='delivery':
            if testnet == True:
                self.http_way = 'http://testnet.binancefuture.com/dapi/v1/'
                self.wss_way = 'wss://dstream.binancefuture.com/ws/'
            else:
                self.http_way = 'http://dapi.binance.com/dapi/v1/'
                self.wss_way = 'wss://dstream.binance.com/ws/'
